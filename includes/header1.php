...
<body class="bg-slate-50 text-slate-900 min-h-screen">
    ...
    <nav class="flex justify-between items-center px-6 md:px-12 py-5 bg-white border-b border-slate-100 sticky top-0 z-50">
        <div class="flex items-center gap-2 cursor-pointer" onclick="window.location.href='../index.php'">
            <div class="w-9 h-9 bg-indigo-600 rounded-xl flex items-center justify-center text-white font-black shadow-lg shadow-indigo-100">W</div>
        ...
        <div class="hidden md:flex items-center gap-8 text-sm font-bold text-slate-500">
            <?php if($current_role === 'driver'): ?>
                ...
                <a href="../passenger/offer_ride.php" class="text-indigo-600">+ Post Trip</a>
            <?php else: ?>
        ...
        <div class="flex items-center gap-4">
            <div class="hidden sm:block text-right">
                <p class="text-[10px] font-black text-slate-300 uppercase"><?php echo $current_role; ?></p>
                <p class="text-xs font-bold text-slate-700"><?php echo htmlspecialchars($user_name); ?></p>
            </div>
            <a href="../actions/logout.php" class="p-2.5 bg-rose-50 text-rose-500 rounded-xl hover:bg-rose-500 hover:text-white transition-all">
