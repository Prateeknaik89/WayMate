...
<body class="text-slate-900 min-h-screen relative overflow-x-hidden">
    ...
    <nav class="flex justify-between items-center px-6 md:px-12 py-4 bg-slate-900 text-white border-b border-slate-800 sticky top-0 z-50 shadow-2xl">
        ...
        <div class="flex items-center gap-10">
            ...
            <div class="hidden lg:flex items-center gap-6 mt-1">
                <a href="dashboard.php" class="text-[10px] font-black uppercase tracking-widest transition-colors <?php echo $current_page == 'dashboard.php' ? 'text-white' : 'text-slate-400 hover:text-white'; ?>">
                    Dashboard
                </a>
                <a href="admin_reviews.php" class="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest transition-colors <?php echo $current_page == 'admin_reviews.php' ? 'text-white' : 'text-slate-400 hover:text-white'; ?>">
                    Feedback Vault
                    <?php if($current_page != 'admin_reviews.php'): ?>
                        <span class="bg-indigo-500 text-white px-1.5 py-0.5 rounded-md text-[8px] animate-pulse">New</span>
                    <?php endif; ?>
                </a>
            </div>
        </div>
